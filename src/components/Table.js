...
function Table({ query, headers, rows, csvData }) {
  return (
    <div>
      {query ? (
        <section className="text-white body-font pl-4">
          <div className="flex w-full justify-between mt-6 lg:mt-0">
            <div className="font-bold text-center py-4 w-40 bg-gray-500 text-white rounded-tl-md rounded-tr-lg text-xl">
              Result
            </div>
            <CSVLink
              data={csvData}
              ...
              className="p-2"
            >
              <button className="flex mx-auto text-black bg-violet-500 border-0 py-2 h-11 px-4 focus:outline-none hover:bg-green-600 hover:text-white rounded text-lg justify-center items-center">
                <svg
                ...
                <span className="pl-2 font-semibold">Download</span>
              </button>
            </CSVLink>
          </div>
          <TableUI headers={headers} rows={rows} csvData={csvData}/>
        </section>
      ) : (
        <div className="w-full flex text-center h-80 justify-center items-center font-bold font-mono text-white text-2xl px-6">
        </div>
      )}
    </div>
  );
